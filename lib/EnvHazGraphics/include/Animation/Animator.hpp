...
class Animator
{
  public:
    ...
    void Update(float deltaTime)
    {
        ...
        size_t frameIndex = 0;
        while (frameIndex < anim.frames.size() - 1 && animationTime > anim.frames[frameIndex + 1].timeStamp)
            frameIndex++;

        const KeyFrame &frameA = anim.frames[frameIndex];
        const KeyFrame &frameB = anim.frames[(frameIndex + 1) % anim.frames.size()];

        float t = (animationTime - frameA.timeStamp) / (frameB.timeStamp - frameA.timeStamp);
        ...
        for (size_t i = 0; i < frameA.transforms.size(); ++i)
        {
            ...
            glm::quat rotation = glm::normalize(glm::slerp(tA.rotation, tB.rotation, t));

            glm::mat4 mat = glm::translate(glm::mat4(1.0f), position) * glm::mat4_cast(rotation) *
