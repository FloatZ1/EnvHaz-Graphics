...
struct Joint
{
    ...
    glm::mat4 GetDeformedBindMatrix()
    {
        ...
        return glm::translate(glm::mat4(1.0f), m_DeformedNodeTranslation) * glm::mat4(m_DeformedNodeRotation) *
               glm::scale(glm::mat4(1.0f), m_DeformedNodeScale) * m_UndeformedNodeMatrix;
    }
...
class AnimatedModel
{
  ...
  public:
   ...
   /* void SetShaderID(ShaderComboID id)
    ...
    void SetAnimatorID(int id)
    {
        animatorID = id;
    }

    int GetAnimatorID() const
    {
        return animatorID;
    }

    int GetSkeletonID() const
    {
        return skeletonID;
    }
...
class AnimatedModelManager
{
  ...
  private:
    ...
    std::unordered_map<eHazGraphics_Utils::HashedString, AnimatedModel> loadedModels;
