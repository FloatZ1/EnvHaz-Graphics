...
class MaterialManager
{
  public:
    ...
    unsigned int CreatePBRMaterial(
        unsigned int albedoID, unsigned int prmID, unsigned int NormalMapID,
        unsigned int EmissionID); // gets the texture ids and creates a struct in the pbr materials

    void UpdateMaterial(unsigned int MaterialID, PBRMaterial replacement); // in case we change a texture of a material

    std::pair<const std::vector<PBRMaterial> &, TypeFlags> SubmitMaterials(); // prepares the data to be sent to
  ...
  private:
    ...
    std::unordered_map<eHazGraphics_Utils::HashedString, unsigned int> MaterialNames;
};
