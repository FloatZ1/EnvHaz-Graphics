...
public:
  ...
  const size_t GetDataSize() const { return size; }
...
struct ShaderComboID {
  ...
  ShaderComboID(eHazGraphics_Utils::HashedString vs,
  ...
  struct ShaderComboHasher {
    std::size_t operator()(const ShaderComboID &id) const noexcept {
      // simple hash combine
      return id.vertex ^ (id.fragment + 0x9e3779b97f4a7c15ULL +
                          (id.vertex << 6) + (id.vertex >> 2));
    }
  };
...
struct DrawElementsIndirectCommand {
  ...
  bool operator==(const DrawElementsIndirectCommand &other) const {
    return count == other.count && instanceCount == other.instanceCount &&
           firstIndex == other.firstIndex && baseVertex == other.baseVertex &&
...
struct InstanceData {
  ...
  uint32_t materialID;
  uint32_t modelMatID;
  uint32_t numJoints; // only for animations
  uint32_t animMatLocation;
};
